    ...
    <section class="content">
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Sửa cấu hình nội dung</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" action="{{ url('admin/content/edit/'.$cont->id) }}" method="POST" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class="box-body">
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('name') ? ' has-error' : ' has-feedback' }}">
                            <label for="title">Tên nội dung</label>
                            <input type="text" name="name" class="form-control" id="name" value="{{$cont->name}}">
                            @if ($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('code') ? ' has-error' : ' has-feedback' }}">
                            ...
                            <input type="text" name="code" class="form-control" id="code" value="{{$cont->code}}">
                            @if ($errors->has('code'))
                                <span class="help-block">
                        ...
                        <div class="form-group {{ $errors->has('vn') ? ' has-error' : ' has-feedback' }}">
                            ...
                            <input type="text" name="vn" class="form-control" id="vn" value="{{$cont->vn}}">
                            @if ($errors->has('fa'))
                                <span class="help-block">
                        ...
                        <div class="form-group {{ $errors->has('fa') ? ' has-error' : ' has-feedback' }}">
                            ...
                            <input type="text" name="fa" class="form-control" id="fa" value="{{$cont->fa}}">
                            @if ($errors->has('fa'))
                                <span class="help-block">
                        ...
                        <div class="form-group">
                            ...
                            <input type="radio" name="category" id="category" value="1" @if($cont->category==1)checked @endif> Websites <input type="radio" name="category" id="category" value="0" @if($cont->category==0)checked @endif> Systems
                        </div>
                        <div class="form-group">
                            <label for="title">Trạng thái</label><br>
                            <input type="radio" name="status" id="status" value="1" @if($cont->status==1)checked @endif> Kích hoạt <input type="radio" name="status" id="status" value="0" @if($cont->status==0)checked @endif> Ngừng kích hoạt
                        </div>
                        <div class="form-group">
                            <label for="title">Thứ tự</label>
                            <input type="text" name="order" class="form-control" id="order" value="{{$cont->order}}">
                        </div>
                        <!-- /.box-body -->
