            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">Sửa cấu hình</h3>
                    </div>
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form role="form" action="{{ url('admin/userlevel/edit/'.$level->id) }}" method="POST" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        <div class="box-body">
                            ...
                            <div class="form-group">
                             {{--   <label for="title">Mã cấu hình</label>--}}
                                <input type="hidden" name="code" class="form-control" id="code" value="{{$level->code}}">
                            </div>
                            <div class="form-group">
                                <label for="title">Giá trị</label>
                                <input type="text" name="value" class="form-control" id="value" value="{{$level->value}}">
                            </div>

                            <div class="form-group">
                                <label for="title">Danh mục cấp độ</label>
                                <select class="form-control" name="categories">
                                    <option value="{{$level->cate_id}}">{{$level->cates->name}}</option>
