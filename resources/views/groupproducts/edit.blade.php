...
@section('content')
    ...
    <section class="content">
        <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" action="{{ url('admin/groupproducts/edit/'.$gp->id)}}" method="POST" enctype="multipart/form-data">
                    {!! csrf_field() !!}
                    <div class="box-body">
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="title">Tên nhóm sản phẩm</label>
                                <input type="text" name="name" class="form-control" id="name" value="{{$gp->name}}">
                            </div>

                            <div class="form-group">
                                <label for="title">Block hiển thị</label>
                                <select class="form-control" name="block" id="block">
                                    <option value="{{$gp->block}}">@if($gp->block=='trangchu')Trang chủ @else Sản phẩm @endif</option>
                                    <option value="trangchu">Trang chủ</option>
                                    <option value="sanpham">Sản phẩm</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="title">Loại dữ liệu gán</label>
                                <select class="form-control">
                                    <option disabled selected>{{$gp->data}}</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label class="radio-inline">
                                    <input name="status" value="1"  @if($gp->status==1) checked="checked" @endif  type="radio">
                                   Hiển thị</label>
                                <label class="radio-inline">
                                    <input name="status" value="0" @if($gp->status==0) checked="checked" @endif type="radio">
                                    Ẩn</label>
                            </div>
    ...
    <script>
        $('#data').change(function(){
            ...
            if(data=="categories")
            {
                ...
                if(nowdata=='categories')
                {
                    $.ajax({
                        url: '{{url('admin/editgroupsproducts')}}',
                        dataType: "json",
                        type: "post",
                        data: {_method: 'post', _token: '{{csrf_token()}}', pon: couponid}
                    }).done(function(jsonData) {
                        var opt = "";
                ...
                else
                {
                    $.ajax({
                        ...
                        type: "post",
                        data: {_method: 'post', _token: '{{csrf_token()}}', pon: couponid}
                    }).done(function(produts) {
                        $("#renderarticle").css("display","block");
                        $('#article').html(produts);
                    }).fail(function(produts) {
                        alert('error send data');
                    });
                }
